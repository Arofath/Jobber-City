    ...
    <div class="bg-white rounded-2xl w-full max-w-md" style="box-shadow:0 4px 16px 0 rgba(16,24,40,0.15)">
      ...
      <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-4">
        ...
        <div>
          ...
          <select
            ...
            :class="{ 'border-red-400 bg-red-50': errors.province_id }"
          >
          ...
          <p v-if="errors.province_id" class="text-xs text-red-500 mt-1">{{ errors.province_id }}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            class="w-full px-4 py-2.5 text-sm bg-surfaceorder border-surface-borderorder rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-300 focus:border-brand-400 transition"
        ...
        <div class="flex items-center justify-between py-1">
          <div>
            ...
            <p class="text-xs text-gray-400">Inactive districts are hidden from users</p>
          </div>
          <button
            type="button"
      ...
      <div class="flex items-center gap-3 px-6 py-4 border-t border-surface-border">
        ...
        >
          ...
          {{ saving ? 'Saving...' : isEdit ? 'Save Changes' : 'Add District' }}
        </button>
      </div>
...
const props = defineProps({
  ...
  defaultProvinceId: { type: String,  default: '' },
  saving:            { type: Boolean, default: false },
  saveError:         { type: String,  default: '' },
  // 🎯 បន្ថែម Prop ថ្មីសម្រាប់បញ្ជូនចំនួន District សរុប
  totalCount:        { type: Number,  default: 0 },
})

const emit = defineEmits(['close', 'saved'])

const isEdit = computed(() => !!props.district?.id)
