<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="font-display text-lg font-bold text-gray-900">Job Levels</h2>
        <p class="text-[13px] text-gray-500 mt-0.5">Manage job experience levels</p>
      </div>
    </div>

    <!-- Error -->
    <div v-if="store.jobLevelsError" class="px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-sm text-red-600 flex items-center gap-2 shadow-sm">
      <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z"/>
      </svg>
      {{ store.jobLevelsError }}
    </div>

    <!-- 🟢 KPI Cards (ប្រើ Component រួម) -->
    <MasterDataKpi 
      title="Job Levels" 
      :data="store.jobLevels" 
      v-model="currentTab" 
    >
      <template #icon>
        <!-- Icon សម្រាប់ Job Levels -->
    ...
    <div class="bg-white rounded-2xl p-5 border border-surface-border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
      <div class="relative w-full md:flex-1 max-w-xl flex items-center gap-2">
        ...
        <button @click="store.fetchJobLevels()" class="w-10 h-10 shrink-0 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition" title="Refresh Data">
          <svg class="w-4 h-4" :class="{ 'animate-spin': store.jobLevelsLoading }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
          </svg>
        </button>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-50">
            <!-- Loading -->
            <template v-if="store.jobLevelsLoading">
              <tr v-for="n in 3" :key="n">
                <td colspan="5" class="px-5 py-4 text-center text-gray-400">Loading...</td>
              </tr>
            </template>
            <!-- Empty -->
            <tr v-else-if="filteredItems.length === 0">
              <td colspan="5" class="px-5 py-14 text-center text-gray-400">No job levels found.</td>
            </tr>
            <!-- Rows -->
            <tr v-else v-for="(item, index) in filteredItems" :key="item.id" class="hover:bg-gray-50/50 transition-colors">
              ...
              <td class="px-5 py-4">
                <span class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-brand-50 text-brand-600 text-xs font-bold">{{ item.order }}</span>
              </td>
              <td class="px-5 py-4">
    ...
    <!-- Modal -->
    <JobLevelFormModal v-if="showModal" :item="selectedItem" :saving="saving" :save-error="saveError" :total-count="store.jobLevels.length" @close="closeModal" @saved="handleSaved"/>
  </div>
</template>
...
import JobLevelFormModal from './JobLevelFormModal.vue'
import MasterDataKpi from '../../components/ui/MasterDataKpi.vue' // 🎯 ហៅ Component មកប្រើ

const store = useMasterDataStore()

// 🎯 States ថ្មីសម្រាប់ Filtering Frontend
...
// ទាញយកទិន្នន័យទាំងអស់ពេលបើកទំព័រ
onMounted(() => {
  store.fetchJobLevels() 
})

// 🎯 Filter Logic 
const filteredItems = computed(() => {
  return store.jobLevels.filter(item => {
    let matchesTab = true;
...
async function handleSaved(data) {
  ...
  try {
    if (data.id) {
      await store.updateJobLevel(data.id, data)
      toastSuccess('Updated successfully!')
    } else {
      await store.createJobLevel(data)
...
async function handleDeleteClick(item) {
  ...
  try {
    await store.removeJobLevel(item.id)
