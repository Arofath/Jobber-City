<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="font-display text-lg font-bold text-gray-900">Skills</h2>
        <p class="text-[13px] text-gray-500 mt-0.5">Manage all available skills for job postings</p>
      </div>
    </div>

    <div v-if="store.skillsError" class="px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-sm text-red-600 flex items-center gap-2 shadow-sm">
    ...
    >
      <template #icon>
        <!-- Icon សម្រាប់ Skills (រូបសញ្ញាផ្កាយ/ភ្លើង) -->
    ...
    <div class="bg-white rounded-2xl p-5 border border-surface-border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
      <div class="relative w-full md:flex-1 max-w-xl flex items-center gap-2">
        <div class="relative w-full">
          ...
          <input v-model="searchQuery" type="text" placeholder="Search skills..." class="w-full pl-10 pr-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-300 transition-all" />
        </div>
        <button @click="store.fetchSkills()" class="w-10 h-10 shrink-0 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition"><svg class="w-4 h-4" :class="{ 'animate-spin': store.skillsLoading }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg></button>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-50">
            <template v-if="store.skillsLoading"><tr v-for="n in 3" :key="n"><td colspan="5" class="px-5 py-4 text-center text-gray-400">Loading...</td></tr></template>
            <tr v-else-if="filteredItems.length === 0"><td colspan="5" class="px-5 py-14 text-center text-gray-400">No skills found.</td></tr>
    ...
    <!-- Modal -->
    <SkillFormModal v-if="showModal" :item="selectedItem" :saving="saving" :save-error="saveError" :total-count="store.skills.length" @close="closeModal" @saved="handleSaved"/>
  </div>
</template>
...
const filteredItems = computed(() => {
  return store.skills.filter(item => {
    let matchesTab = true;
...
async function handleSaved(data) {
  ...
  try {
    if (data.id) {
      await store.updateSkill(data.id, data)
...
async function handleDeleteClick(item) {
  ...
  try {
    await store.removeSkill(item.id)
